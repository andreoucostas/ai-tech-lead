...
| Claude Code | Root `CLAUDE.md` importing `@AGENTS.md` consumed (B-241 root-mirror inversion) | `canary-import-resolution.ps1 -ImportTarget AGENTS.md -Model haiku` in a fresh temp fixture: root `CLAUDE.md` carried only the `@AGENTS.md` line, the random sentinel lived only in `AGENTS.md`, the model echoed it, transcript showed zero file-access tool invocations, control count of the sentinel in `CLAUDE.md` = 0. Launched from inside a Claude Code session with the nesting variable blanked. Not run in this pass: a no-import negative control on the same host | Claude Code 2.1.260 / PowerShell 7.6.6 | 2026-09-16 — local Windows, `-p` non-interactive, treatment + in-fixture control only; certifies host import assembly for a root file, not model adherence |
