...
| Claude Code | Root `CLAUDE.md` importing `@AGENTS.md` consumed (B-241 root-mirror inversion) | Treatment: `canary-import-resolution.ps1 -ImportTarget AGENTS.md -Model haiku` in a fresh temp fixture — root `CLAUDE.md` carried only the `@AGENTS.md` line, the random sentinel lived only in `AGENTS.md`, the model echoed it, zero file-access tool invocations, sentinel count in `CLAUDE.md` = 0. Negative control (`-OmitImport`, added after independent review): the same fixture without the import line returned `NOT-IN-CONTEXT` with zero tool invocations (`CONTROL VALID`), so the target is not loaded by another route and the positive is attributable to `@import`. Positive control: the framework-rules import row above. Both launched from inside a Claude Code session with the nesting variable blanked | Claude Code 2.1.260 / PowerShell 7.6.6 | 2026-09-16 — local Windows, `-p` non-interactive, treatment + negative control; certifies host import assembly for a root file, not model adherence |
| Claude Code | `permissions.deny` `Edit(/dist/**)` honoured against a file-editing tool | A fresh `-p` session (haiku, `--allowedTools Read,Edit`, cwd = this repo) was asked to append a line to `dist/dotnet/README.md` with the Edit tool; it reported `Denied. Exact message: "File is in a directory that is denied by your permission settings."` and the file was byte-identical afterwards. The host also warned at session start that a `Write(/dist/**)` rule "is not matched by file permission checks — only Edit(path) rules are", so that rule was removed and `MetaHooks` now refuses the shape | Claude Code 2.1.260 | 2026-09-16 — local Windows, `-p` non-interactive; certifies the deny rule fires for the Edit tool, not that the rule is enforcement (`& git push`-style bypasses are documented) |
...
| Claude Code | UserPromptSubmit route rails fired and consumed | B-253 probe: a patched hook wrote a sentinel matching each run's own init `session_id` with `intent=fix`, and the agent quoted a token that exists on disk only in pieces in its first message, before any tool call, 3/3; no tool call touched the hook or showed the probe code, 0/3 (`meta/eval-results.md`) | Claude Code 2.1.281 / claude-sonnet-5 | 2026-09-24 — local Windows, `-p` non-interactive, route-fix prompt only; certifies firing and consumption, not adherence |
...
| Copilot CLI | Ordinary scoped-knowledge navigation and application | On one synthetic feature-only prompt with absent hooks, typed events showed direct index/body/retail-policy reads and the patch passed a calibrated hidden grader at 7/0. Post-review found the manually prepared wiki nonconforming (installed wiki check: 37 failures), so this records behavior but does not certify product-conformant acceptance, knowledge causation, the 30-entry hook threshold, or plan-rail compliance | 1.0.83 operator-recorded; raw route metadata: GPT-5.4 medium | 2026-09-09 — bounded nonconforming synthetic observation; not certified |
