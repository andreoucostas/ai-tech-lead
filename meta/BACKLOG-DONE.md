...
> **DONE 2026-09-04 in v0.83.0.** Native Windows is now the sole supported host; PowerShell 7 is
> primary and Windows PowerShell 5.1 remains the runtime fallback. Framework-owned execution and
> registration surfaces contain no active Bash. Four inert historical canaries and textual history
> remain deliberately. Each distribution registers exactly 18 PowerShell hooks, and all eight root
> plus three-distribution Windows CI contexts passed under both supported hosts before tagging.
...
guard failures. Its narrowly necessary historical/test exceptions are exact path-and-digest pairs
and every application is printed.
...
Opus at xhigh effort. Opus found release-range guard, legacy-hook recognizer, caller-proof, CI-scope,
historical-anchor, binary, migration, tag-inspection and record gaps; each was reverified and
corrected. A Terra correction review then found that BOM-prefixed PowerShell containing NUL could
take the generic binary skip; the guard now refuses it on both hosts. The final Terra confirmation
approved the exact reviewed intermediate-blob exception and corrected the premature closure record.
The release commit `4f9357f` passed GitHub Actions run 33907806431 across all eight contexts and tag
`v0.83.0` was pushed and confirmed without a waiver.
