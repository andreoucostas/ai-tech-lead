...
## v0.79.1 strategic re-triage closures (2026-08-31)

These records moved only after each remaining obligation was completed or explicitly rejected on current evidence. Their prior histories are retained below; age, blockage, and `PARTIALLY DONE` status were not closure reasons.

### B-96 · `map-warehouse` maps the ETL, not the warehouse
> **CLOSED 2026-08-31 — the sole residual experiment is explicitly retired, not replaced.** Its
> frozen enriched-map arm requires Arm 1's machine-produced map verbatim, while the current runner
> has no produced-map input and its enriched fixture is maintainer-authored. The old host/model/
> product comparison cannot be reconstructed; later warehouse runs are not evidence of B-96's
> effect. Reopen only for a current decision with a pre-registered same-run control, immutable
> produced-map handoff, and red-tested grader.
...
> **CLOSED 2026-08-31 — assisted migration is the deliberate answer to the remaining population.**
> The update-delivered carrier, doctor `PENDING` row, session discovery, and README already state the
> one-time manual migration and preserve consumer-owned `CLAUDE.md`; the unresolved precedence risk
> is disclosed rather than assumed benign. Reopen only if a supported host stops delivering or
> discovering the carrier, or direct evidence shows the documented migration unsafe or materially
> unusable. Do not resurrect protected-file overwrite.
...
> **CLOSED 2026-08-31 — the runtime/architecture premise is stale.** B-170 removed local shipped-
> suite duplication, the runner already emits per-file timings, and exact-baseline CI run
> `33367125653` passed all eight required jobs without runtime failure. Reopen only on repeated
> configured local-budget breaches on an idle maintainer host or observed CI process contention that
> causes release failure; use the existing timings before redesigning process isolation.
...
> **CLOSED 2026-08-31 — the investigation is retained and a permanent executor is rejected.** The
> census found 34/42 candidate-portable scenarios and eight typed-event-dependent ones. Codex 0.149.0
> has headless JSONL/schema/last-message controls but no native spend or timeout flag; WSD-054 already
> demonstrated one-off artifact grading. B-129, the observed budget harm, is routing-dependent and
> excluded, so permanent dual integration removes none of it. Reopen only when repeated artifact-only
> work demonstrates concrete ad-hoc executor cost or defects and has an immutable red-testable oracle.
...
> **DONE 2026-08-31 — the product defect was already fixed; the residual authoring rule is now
> canonical.** Root Maintenance rule 7 requires lifecycle position as explicit gate input and rejects
> ambient discovery; it remains guidance because no generic detector can distinguish deliberate
> lifecycle inputs honestly. Reopen on a second live-state gate defect or a cheap discriminating
> structural boundary.
...
> **DONE 2026-08-31 — all four semantic inertness shapes now sit beside Maintenance rule 4.** The
> active guidance covers literal inert assertions, exit-domain collision, empty/absent conflated
> with cannot-examine, and normalization/comparison that stops comparing. A generic mutation
> framework remains disproportionate. Reopen if an unaddressed shape recurs and a shared subject-
> level mutation seam is cheaper than per-suite machinery.
...
> **DONE 2026-08-31 — the two platform legs and the missing scope question are explicit.** The
> maintainer guide identifies Windows/Linux independently from PowerShell-host editions and now asks
> whether every changed function belongs to the frozen contract, with unrequired changes justified
> or reverted. No proxy or third generic CI leg was added. Reopen on another escape through that
...
> **DONE 2026-08-31 — the reviewer disciplines are now active maintainer guidance.** Verification
> trusts the process exit, proves filters on known positive and negative controls, and compares bytes
> or hashes rather than rendered output for equality claims. No checker-of-checkers was added. Reopen
> for a distinct reviewer false-green mechanism outside those three disciplines.
...
> **CLOSED 2026-08-31 — the proposed cross-reader/test growth is hypothetical.** The framework-owned
> catalog is currently unique and is not a consumer extension surface, so four-reader normalization
> and a permanent mutation remove no observed defect. Reopen if an actual duplicate/disagreement
> appears or the catalog becomes externally extensible.
...
> **CLOSED 2026-08-31 — WSD-065 records deliberate non-support.** Documented executed-script
> invocations on Windows/Linux remain supported, including the exact `bash -e` docs-sync promise;
> sourcing scripts or deliberately exporting `SHELLOPTS=errexit` into their process tree is outside
> the public contract. Reopen only for a documented invocation that exports it or a supported-host
> consumer incident under a published recipe.
...
> **CLOSED 2026-08-31 — the old local-container verification premise is superseded.** The shipped
> reference recipes separate framework-state and repository-evidenced code legs and explicitly put
> the real failing-PR/merge-lock oracle in the consumer's Bitbucket environment. A maintainer Jenkins
> container cannot prove site-specific plugins, labels, toolchains, status reporting, or merge
> blocking. Reopen for a concrete consumer recipe defect or a portable framework-owned oracle.
...
> **CLOSED 2026-08-31 — the generic testing-program premise no longer justifies its cost.** The cited
> plan is absent, no observed escaped defect identifies coverage/mutation infrastructure as the
> smallest diagnostic gap, and recreating an L-sized cross-stack program would be test growth without
> a decision. Reopen only when a repeated defect class escapes focused gates specifically because
> changed production paths are unmeasured and a smaller direct oracle cannot close it.
...
> **CLOSED 2026-08-31 — completed protections retained; remaining probe work rejected as
> disproportionate.** The `formInputs` defects were repaired and Maintenance model rule 4 now
> carries the general red/green instrument obligation. Commit `5571d1a` also split `cvaInterface`
> from `cvaProvider`, making the two mechanisms visible independently. A dedicated provider-plus-
> `NgControl` self-test was not added, and the result still accepts that combination; the prompt
> also remains telegraphing. Those residuals are deliberately not implemented:
> `angular-form-control` is explicitly labelled `SATURATED`, no current decision consumes its
> verdict, and B-66 already shipped from stronger field evidence with behavioral effect honestly
> marked unmeasured. Strengthening a nondiscriminating historical instrument would add test cost
> without reducing observed harm. A future forms decision must begin with a newly discriminating,
> pre-registered scenario rather than reopening this one.
>
> **RCA.** Parser gates cannot inspect experimental meaning, while the original self-test exercised
> syntax cases authored from the same assumptions as the grader. It therefore missed both an OR-
> combined signal that erased mechanism identity and a prompt that supplied the answer. The same
> class exposes any behavioral grader that conflates alternative mechanisms or any fixture/prompt
> that states the conclusion it scores. Maintenance rule 4 and explicit scenario dispositions are
> the durable control; a generic expansion of this retired probe is not.
...
> **CLOSED 2026-08-31 — the standing rule is complete; the retrospective scenario inventory is
> rejected as unbounded work.** Maintenance model rule 4 now requires a release-specific red
> observation and a constructible green world whenever an outcome measure is used. The remaining
> sweep has grown from 15 scenarios to 44, while only two currently carry `measures` dispositions;
> maintaining a universal saturation/reachability catalogue would become a second, perpetually
> stale backlog. The decision-relevant `docs-tier` question—whether on-demand documentation can
> arrive—was already answered; pointer causality remained unresolved but no decision depends on it.
> The record also contains a valid `archived-redirect` run with every conjunct now used by the
> repaired operational measure true, although it was not regraded under the repaired code. A bare
> `route-fix` arm matters only if B-44 later triggers an actual retirement decision. Future
> experiments must satisfy rule 4 when they become decision-relevant; they do not justify
> retroactive live spend now.
>
> **RCA.** No parser or harness self-test can establish that an outcome measure points at the right
> behavioral question. The earlier sweep was a snapshot that silently became incomplete as the
> scenario catalogue expanded. Every behavioral scenario remains exposed to unreachable,
> saturated, telegraphed, or attribution-blind measures, but the proportionate remedy is per-
> instrument proof at the point of use—not a generic, ever-growing audit programme.
...
> **CLOSED 2026-08-31 — no shipped change; the stale paid probe is retired without reclassifying
> either void run.** WSD-042 authorised only a routing-gated pair. Both attempts remain
> `VOID`/`INCOMPLETE`; neither proves carrier reachability, baseline reachability, or content effect.
> They cost $18.3126324 in total and produced zero scoreable results. The old implementation is no
> longer a current instrument: commit `80c789e` is not an ancestor of v0.79.1, while the runner has
> changed `+380/-143`, the scenario catalogue `+18/-19`, and each `map-warehouse` mirror `+8/-18`.
> A valid continuation would therefore require a new lock, a port, another paid 16-trial batch, and
> a positive routing threshold before any content A/B. No wrong publication outcome caused by the
> current framework has been observed, so that cost exceeds expected value under WSD-057. Reopen
> only for a concrete current framework-caused publication-surface error.
>
> The second-run sidecar was preserved verbatim in `meta/eval-results.md` by commit `3b9bd6c`, then
> deleted from the clean sibling worktree after its branch (`codex/b129-publication-routing-probe`),
> commit (`80c789eadc1a6772fb9ef89be8639a42bd19c0a7`), sole untracked path, 3,289-byte length, and
> SHA-256 `516F7F13F434D82EA52D393372383133889CAF0F03A96E6A307A0E1D8AA7A717` were independently
> verified. The sibling branch and commit were not changed.
>
> **RCA.** No artifact defect or parser-gate miss is claimed. The process gap was portfolio
> revalidation: a blocked-on-spend entry survived two void runs and substantial product/harness
> drift without a fresh value decision. An honest instrument can prevent a false conclusion but
> cannot decide whether another run is worth its cost. The same class exposes stale, unmerged live-
> evaluation branches whose locked decision and current product have diverged; revalidate premise,
> ancestry, instrument drift, and expected decision value before spending again.
...
> **REJECTED 2026-08-31 — the premise still lacks observed harm.** The repository contains no field
> report of a durable learning being lost because promotion was not offered. Existing guidance
> already tells the agent to offer `remember-for-team` when a team-worthy learning surfaces, and
> `/docs-sync` explicitly asks whether an existing durable learning deserves promotion. Phase 0
> would measure whether an agent offers promotion, not whether a real team lost valuable knowledge—
> the harm the item claims to prevent. Preserve the reviewed prohibitions against IDs, counters,
> receipt blocks, threshold parsers, hook writes, automatic promotion, and reuse-derived truth.
> Reconsider only after B-42 or another concrete field observation identifies an actual missed
> promotion.
>
> **RCA.** No gate missed a product defect: the entry was filed from a plausible reliance-on-memory
> concern and explicitly recorded that no consumer miss had been observed. The exposed class is
> speculative instrumentation or always-loaded guidance added before a field outcome establishes
> value. Require concrete harm and a decision-changing measurement before reopening that class.
...
> **REJECTED 2026-08-31 — the claimed invocation path does not exist.** The always-loaded workflow
> carrier defines Feature, Bug fix, Refactor, Test, Investigation/design, and Debt rails; it does
> not direct the model to invoke `/review` or dispatch five auditors. `route-prompt` recognizes
> review only when the prompt explicitly matches `review this`, related review phrases, or
> `quality gate`, and its injected review rails contain no fan-out instruction. Only
> `.claude/commands/review.md` requires spawning all five subagents. A diff-shaped prompt that
> deliberately never says “review” therefore cannot test the carrier dependency described by this
> item; it would instead measure unprompted model-selected command escalation. No observed field
> miss or pre-registered action makes that a current decision.
>
> **RCA.** No gate validates causal narratives in backlog prose. The entry conflated artifact
> availability, intent classification, command selection, and command execution. The same reasoning
> error exposes claims that a visible command, skill, or frontmatter description was necessarily
> invoked. Future routing work must trace the exact carrier and typed invocation path before
> proposing behavioral measurement.
...
> **CLOSED 2026-08-31 — the static investigation is complete; the global live arm and maintenance-
> model bar are rejected.** The audit correctly reproduced the extreme cases and failed in the
> middle, so it remains only a candidate-ranking proxy. The observed read-side non-reach already
> has a proportionate remedy through B-98/WSD-032's measured carrier. Load-shaped prompts selected
> `add-warehouse-load` 6/6 in B-117, while B-126 selected it only 1/6 but still produced the correct
> behavioral outcome in every counted run; selection rate alone therefore does not identify
> consumer harm. No non-warehouse miss or decision threshold justifies further live spend, and a
> global selection bar would conflate task shape, host, vocabulary, and outcome. Future skill work
> should start from a concrete missed outcome and pre-register a task- and host-specific action
> threshold.
>
> **RCA.** Structural gates can prove that skills exist, mirror correctly, and are discoverable,
> but model selection is probabilistic and host-specific. Every selectively routed skill is exposed
> to non-reach, yet absence of a routing measurement is not itself a defect. The proportionate
> response is an evidence-triggered, outcome-based probe for a concrete miss—not universal routing
> telemetry or a speculative new-skill threshold.
...
> **CLOSED 2026-08-31 — rejected as disproportionate after implementation and independent
> adversarial review.** The malformed JSON worlds are constructible, but the record contains no
> field incident involving duplicate/case-colliding members, wrong roots, nested decoy
> registrations, or a `jq` probe/query mismatch. The attempted correction grew to eight artifacts
> and `+1163/-222` lines without adding a suite or `It`: honest cross-parser agreement required
> typed provider outcomes, lossless value framing, fallback semantics, recursive schema positions,
> ordinal decoded-string comparison, and control-character handling. The experiment remains
> auditable in commits `73b7fc7` and `a2cd24b`; commits `0986cd6` and `0f256aa` restore the pre-
> experiment product.
>
> **Why no narrower fix:** an isolated root-shape or decoy-property check is technically possible,
> but no observed harm selects one synthetic edge or justifies its permanent behavior and test
> burden. It would also leave the original cross-parser contract unresolved and could merely move
> false diagnoses between parser paths. Reopen for a concrete Windows/Linux consumer incident
> attributable to one of these states, or if these JSON files become an externally authored
> compatibility boundary where such malformed input is expected; then freeze the smallest artifact-
> specific correction against that observed failure.
>
> **RCA.** The strict-JSON matrices were designed to align accepted syntax, not to establish that
> every constructible semantic edge has product value. They correctly stayed green on canonical
> artifacts and cannot judge field prevalence or maintenance cost. The backlog entry promoted
> parser divergence to harm and estimated from the visible checks before tracing decoded values
> through provider selection, shell framing, PowerShell comparison semantics, and downstream
> diagnostics. The exposed class is parser-hardening work justified only by adversarial malformed
> fixtures. The proportionate control is evidence-first premise/design critique, not another
> speculative gate.
...
> **IMPLEMENTED AND CLOSED 2026-09-01 for v0.80.0.** The exact frozen Step 6 block is the
> only normative source change. Its LF-normalized UTF-8 identity is 470 bytes and SHA-256
> `f6cd9c822371970831b93e6ee9d05d50d24ac9e6ca1369eb99eb877f12868fb6` in the canonical source and
> all three composed distributions, exactly seven bytes below the frozen 477-byte predecessor.
> Static Claude context therefore moves 39,518→39,511 bytes for dotnet, 38,041→38,034 for angular,
> and 47,034→47,027 for monorepo. No second normative surface, command, skill, hook, test, eval,
> artifact inventory, or warehouse/stack trigger changed. These facts prove structural delivery
> only; behavioral compliance remains **UNMEASURED**.

> **Implementation RCA.** No parser or composition gate caught the old report-only completion
> contract because it was valid Markdown, faithfully composed, within budget, and identical across
> distributions. Those gates prove structure and delivery, not that completion prose owns repository
> truth invalidated by the task. The same class exposes semantically valid workflow instructions that
> permit reporting known stale state instead of reconciling it, especially living artifacts with
> distinct ownership, evidence, history, and security constraints. The proportionate control is
> causal same-task reconciliation plus independent semantic scope review; no deterministic model-
> compliance claim is made, and behavior remains **UNMEASURED**.
...
> **IMPLEMENTED AND CLOSED 2026-09-04 for v0.83.0.** Native Windows is now the sole supported host;
> PowerShell 7 is primary and Windows PowerShell 5.1 remains the runtime fallback. Framework-owned
> execution and registration surfaces contain no active Bash. Four inert historical canaries and
> textual history remain deliberately. The distributions register exactly 18 PowerShell hooks and
> CI exposes eight Windows contexts: root plus all three distributions under both supported hosts.

The delivery removed 80 active `.sh` files and two shell-only maintainer launchers, Linux CI,
dual-language registrations, and unsupported portable provider examples. Claude settings select
the PowerShell tool, `!` shell, and command-hook shell independently; Copilot retains six explicit
`pwsh -NoProfile -File` registrations. The installer owns composition policy and provides a
compatibility-only `-GitHooks` refusal until B-220 removes that parameter. The new outgoing-commit
guard checks remote-relative subjects and every changed blob, including every merge parent, before
push and refuses unenumerable remotes, BOM-less PowerShell, NUL-bearing PowerShell, or canonical
guard failures. Its five historical/test exceptions are exact path-and-digest pairs and are printed.

Upgrade handling is content-qualified and conservative. The v0.83 retirement slice contains 18
paths per distribution; its 153 unique released digests were harvested through v0.82 and both
authorities agree. Only an exact path owned by the immediately previous manifest and matching a
released digest can be removed. Modified, unknown, unreadable, wrong-case, reparse, or unverifiable
content is preserved and durably diagnosed. Consumer Git hooks are never deleted; exact legacy hook
dependency closures are preserved and reported. Protected consumer docs, settings, and CI receive
replacement diagnostics without overwrite, with dry-run/apply classification parity.

**Evidence.** Final root aggregates passed 32 suites and 308 cases on both PS7 and native PS5.1;
their case manifests were byte-identical at SHA-256
`79614ED45F6A0ABDFA2B349D6FD4AC3EE907397B7013B31E41E96327CB2929AD`. All three distributions
rebuilt without a freshness diff and passed all 13 validator checks on both hosts: 139/137/145
files, 38 PowerShell files and 18 registrations each. Shipped hook suites passed directly on both
hosts, with byte-identical nonzero manifests (316 dotnet, 311 angular, 316 monorepo). Footprint
schema v2 matched unchanged measurements on both hosts: 39,963/40,000 dotnet, 38,486/40,000 angular,
and 47,479/48,000 monorepo. Live Windows Claude Code and Copilot CLI positive, negative, side-effect,
and process-ancestry canaries are recorded in the locked plan.

The requested review chain used a fresh independent `gpt-5.6-terra` context first, then Claude CLI
Opus at xhigh effort. The first immutable-range pass found no actionable issue. Opus found release-
range guard, legacy-hook recognizer, caller-proof, CI-scope, historical-anchor, binary, migration,
tag-inspection and record gaps; each was reverified and corrected. A Terra correction review then
found that a BOM-prefixed PowerShell blob containing NUL could take the generic binary skip; the
guard now refuses it, with a hostile case green on both hosts. The actual outgoing-range run then
went red because that hostile test embedded its blocked token literally; the fixture now constructs
the token at runtime, preserving the hostile behavior without exempting its source.

**RCA.** No gate challenged the cost premise because Bash parity was itself treated as a standing
quality invariant, while PS5 aggregates resolved children through `pwsh` and silently converted the
fallback leg into another PS7 run. The same false-safety class exposes any matrix whose label names
a host but whose child boundary is not asserted, and any differential oracle that compares two
implementations without revalidating whether both are supported. Exact host reuse, equal nonzero
case manifests, structural PowerShell-only checks, pinned ownership-policy sets, and unchanged
footprint values replace those false signals.

The first outgoing guard also passed synthetic fixtures without being run against the actual range
it governed; structural caller assertions did not prove caller refusal, and its binary convenience
skip accidentally included PowerShell. This exposes every enforcement gate tested only on invented
content or invocation text. The successor controls run the gate on the immutable outgoing blobs,
