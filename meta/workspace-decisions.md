...
- **Date**: 2026-06-04
- **Status**: Accepted (implemented in v0.8.0+); the mirror clause is superseded by WSD-098 (2026-09-24)
...
Validators, CI, and the consumer docs-sync branch consume only zero/nonzero; the numeric count is
already printed for humans. Final local preflight later disproved the broader original census claim:
the template-repo docs-sync branch republishes the child status even though its public header
promises only `0` pass / `1` fail and its existing B-149 mutation result requires `1`. The observed
read-failure world is limited to the marked template/self-host branch because installers exclude
`.template-repo`; installed consumers remain statically exposed to false `MISSING` on other abnormal
checker exits, but no such consumer incident is claimed.
...
## WSD-088: retire the architecture viewer; ordinary overwrite and ledger retirement stay separate (2026-09-16)

B-239's implementation contract is locked in
`.claude/plans/2026-09-16-architecture-retirement-implementation.md` and implemented for v0.87.0.
The runtime viewer is retired rather than hardened. Exact pins plus SRI would have closed the
byte-substitution gap; it was rejected because retention creates a standing obligation to re-earn
browser evidence on every dependency bump, in a repository whose required hosts are PS7 and
Windows PowerShell 5.1 with no browser leg. Retirement costs one migration and one inert file.
This weighs ongoing responsibility against a documented convenience. It does not claim observed
exploitation, measured maintenance savings, non-use, or verified substitute rendering on any
consumer host; the loss of the direct diagram view is stated on the shipped page and in all four
changelogs.

**The reusable constraint is the separation, not the retirement.** A shipped path that must be
*neutralized* on already-installed machines takes ordinary overwrite; a shipped path that must be
*removed* takes ledger retirement. They are mutually exclusive by construction — `install.ps1`
rejects a ledger path that is still in the incoming ownership manifest, and the composer fails the
build for the same reason. Ledger deletion is hash-gated, so it preserves exactly the
consumer-regenerated bytes nobody can recognise; deleting the page would therefore have preserved
precisely the at-risk copies. `docs/architecture.html` is overwritten; the generator is retired.

Retirement diagnostics must be audited as a set, not per path. Retiring one twin silently changed
two existing diagnostics for the other: the `.sh` replacement entry was conditioned on its `.ps1`
still being in the incoming manifest, and the generic residual arm mapped any `.sh` to a `.ps1`
that was itself now retired. Neither was identified by three prior reviews of this change; both
were found by reading the installer at the implementation baseline. When retiring a path, inspect
every branch keyed on its siblings or its extension.

Ownership manifests are generated from the composed tree and are never hand-edited. Known-content
...
## WSD-098: AGENTS.md is the shipped instruction file; CLAUDE.md is a two-import stub (2026-09-24)

**Context.** WSD-002's premise decayed: vendor docs (2026-09-24) show VS Code, Copilot CLI and the cloud agent
read `CLAUDE.md` and `AGENTS.md`, so Copilot loaded the rules and the conventions twice. Claude Code reads
`AGENTS.md` itself only when no `CLAUDE.md` exists, and not on every host configuration.
**Decision (user, 2026-09-24).** Ship `AGENTS.md` as the edited file and `CLAUDE.md` as `@AGENTS.md` plus the
carrier import, never "no CLAUDE.md"; no mirror. Update mode moves the older layout once, only over a
