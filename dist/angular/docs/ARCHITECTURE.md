...
- **Tier 2 — Directed**: `CLAUDE.md` (authored, canonical) is read by Claude Code and supported Copilot agent surfaces; `AGENTS.md` (generated full mirror) serves Codex and GitHub code review, while Cursor reads both. Gemini defaults to `GEMINI.md`; Aider needs explicit read configuration.
...
For portable rules, `CLAUDE.md` is the hand-authored source and its generated carriers are **drift-checked in CI**. Project skills are authored once under `.claude/skills/` and discovered there by Claude Code and supported Copilot skill surfaces.
...
|------|----------------------|-------------|
| `CLAUDE.md` | **Authored** (canonical) | Claude Code, supported Copilot agent surfaces, Cursor |
| `AGENTS.md` | Generated mirror | Codex, GitHub code review, Cursor |
...
| Command | Purpose |
|---------|---------|
| `/bootstrap` | One-time: analyse the codebase (7 parallel passes A1–A7: modules, state, components, RxJS, API, build/test/quality, skill discovery), populate CLAUDE.md + TECH_DEBT.md, generate AGENTS.md + copilot-instructions, and write evidenced project skills under `.claude/skills` |
...
sequenceDiagram
    ...
    H-->>A: script emits branch, recent commits, debt heat, workflow primer
    ...
    A->>H: PostToolUse / postToolUse (if the host fires a supported .ts editor/file-write event in src/)
    H-->>A: script emits tsc --noEmit failures and appends mutable local hook telemetry
...
- **SOLID (mandatory)** — literal classic SOLID: an abstraction/token for **every injected service** (DIP, via `abstract class` token or `interface` + `InjectionToken`) plus SRP/OCP/LSP/ISP. `solid-check` is semantic; dependency-cruiser is scaffoldable and enforces direction only after the consumer wires it into CI. Data carriers are exempt.
...
AGENTS.md                     generated mirror for Codex and GitHub code review; Cursor also reads it
