...
`.claude/commands/rebootstrap.md` is the single source of truth for this workflow. Follow it exactly: pre-flight check → baseline pre-step (stop when nothing changed) → re-analysis (A1–A7; the profile full or incremental) → delta synthesis → diff-aware merge with user confirmation per chunk → re-record the baseline → final report.
