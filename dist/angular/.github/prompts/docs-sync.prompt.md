...
`.claude/commands/docs-sync.md` is the single source of truth. Follow it exactly — all six steps: check `AGENTS.md` against the codebase → check the file generated from it (`copilot-instructions.md`) against `AGENTS.md`, and the `route-prompt` rails against the framework rules → check `LEARNINGS.md` → check `FRAMEWORK-CONTEXT.md` for drift → check `TECH_DEBT.md` against the codebase → present a structured drift report.
