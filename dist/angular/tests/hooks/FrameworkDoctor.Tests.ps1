...
It 'bash twin survives without jq or python3 and reports inactive guard' {
    ...
    $doc=Join-Path $r 'scripts/framework-doctor.sh';$ef=[IO.Path]::GetTempFileName()
    try{
        if($bash-match'\\Git\\bin\\bash\.exe$'){
            # Git-bash (MSYS): symlinks are unreliable and copied exes need their DLLs — copy both.
            $git=Split-Path (Split-Path $bash -Parent) -Parent;$usr=Join-Path $git 'usr/bin'
            foreach($n in 'dirname','sed','grep','sort','paste','head'){Copy-Item (Join-Path $usr "$n.exe") $bin}
            Get-ChildItem $usr -Filter '*.dll'|Copy-Item -Destination $bin
            $runner=Join-Path $usr 'bash.exe';$old=$env:PATH
            try{$env:PATH=$bin;$out=& $runner $doc 2>$ef}finally{$env:PATH=$old}
        }else{
            # POSIX hosts: build the sandbox INSIDE bash — pwsh-created symlinks resolved as
            # "command not found" on the linux CI runner. Setup runs with full PATH; only the
            # doctor invocation sees the restricted one.
            $cmd=('for t in dirname sed grep sort paste head; do ln -sf "$(command -v $t)" "{0}/$t"; done; PATH="{0}" "{1}" "{2}"' -f $bin,$bash,$doc)
            $out=& $bash -c $cmd 2>$ef
        }
        $s=[pscustomobject]@{Exit=$LASTEXITCODE;Out=($out-join"`n");Err=[IO.File]::ReadAllText($ef)}
    ...
    }finally{Remove-Item -Force -ErrorAction SilentlyContinue $ef;Remove-Item -Recurse -Force $r}}
