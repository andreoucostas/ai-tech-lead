...
- **`/rebootstrap` re-analyses only what changed since the last run.** `/bootstrap` now ends by
  ...
  a `project.json` or `package.json` was added or removed, or `angular.json` or `nx.json`
...
- **The README in the framework download now starts with what you get and who does what.** A new
  ...
  step and what it produces. Host and platform limits sit in one Host support section, and
  the instructions for an AI agent installing the framework follow Quick Start. The README is not
...
- **The post-write `tsc --noEmit` check was being dropped the same way.** A type-check that failed
  after a write printed its errors but was never fed back to the agent, which then carried on over
  code that does not compile. It now reaches the agent again.
...
- `/bootstrap`, `/rebootstrap`, `/adopt`, and `/generate-copilot` now run the installed
  ...
  repaired when it is in scope; otherwise the workflow reports failure or `CANT-VERIFY`. Generated
  hazard rows must use a supported bare status token and a resolving repository path.
- `TECH_DEBT.md` retains dismissed proposals separately from active debt. Matching claims stay
  suppressed until materially changed evidence is named; a reopening preserves the prior decision
  and records the evidence delta.
- Brownfield adoption screens mature architecture and ADR documents in place. Clean documents keep
  their bytes, paths, and links; flagged content is quarantined; broken references are reported; and
  competing indexes require your choice.
- Project-specific skill discovery excludes framework-owned files through
  `framework-ownership.json` while retaining eligible consumer-owned and corroborated mixed
  evidence. Finite debt is no longer copied into the always-loaded Boy Scout list.
- Asking why something is technical debt no longer starts debt cleanup. Existing security and Boy
  Scout prompt composition remains intact.
...
`dependency-cruiser` is a scaffoldable backstop and becomes enforcement only after you wire it into
blocking CI. Copilot CLI evidence is dated, while VS Code hooks are Preview, off by default,
organization-gated, and not certified across the full lifecycle.
...
adoption is not yet certified. An explicit `dotnet` selection remains available as an informed
override; ordinary Angular and mixed-repository detection are unchanged.
...
- The `add-tests` description in `AGENTS.md` now names the same TestBed and
  `HttpTestingController` test stack as `CLAUDE.md`.
...
- **Four Angular skills now state when to use them — and when not to.** `add-component`,
  ...
  `add-service`, and shared cross-component state goes to `add-signal-store`.
...
- **`scripts/metrics.sh` now reports the same counters as `scripts/metrics.ps1`.** The bash version
  was missing `tests_skipped_focused` and `tautological_expect`, so the JSON it emitted had a
  different key set from the PowerShell version. Anything consuming that JSON now sees both keys
  regardless of which twin produced it.
...
- **`/bootstrap` and `/adopt` now capture your forms conventions.** Until this release neither
  command had any notion of forms, so the `Conventions` section they write for your repo said
  nothing about them — and an agent working on a form had nothing repo-specific to follow. Both
  commands now author a `Forms` subsection, and `/bootstrap`'s component-design analysis pass looks
  for the things that belong in it: whether you use reactive or template-driven forms, where your
  validators live, and whether any of your components are custom form controls and how they plug
  into the forms API.
...
- **`docs/defaults.md` gains a `Forms` section.** It is deliberately a set of detection notes rather
  than a prescribed default: it tells the analysis what to look for in *your* codebase, in the same
  style as the existing SSR / Hydration section. Your own conventions, once bootstrapped, remain the
  authority.
