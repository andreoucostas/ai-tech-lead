...
Implement a new feature in this repository. Derive its applicable technologies, layers, and validation from repository evidence; do not infer Angular from this framework distribution. Every decision must comply with the conventions and patterns in AGENTS.md.
...
- **Leanness check** (the framework rules (`.github/instructions/framework-rules.instructions.md` › Leanness)): can this fit in existing files? Are you tempted to introduce a new interface / helper / pipe / directive — is there a second consumer in this change-set? If not, defer the abstraction.
