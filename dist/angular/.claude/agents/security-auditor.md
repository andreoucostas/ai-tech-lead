...
1. Receive the parent-supplied `-ScopePath <bundle>` and manifest SHA-256. Recompute `manifest.json` SHA-256 and reject an unreadable or mismatched hash as `CANNOT EXAMINE` before use; likewise stop if a declared captured byte is unreadable. From its manifest and declared captured bytes, use repository evidence to establish whether the Angular profile applies. Only when it does, scope to captured `*.ts`, `*.html`, `*.scss`, `*.css`, `*.json` (env / config), `package.json`. Skip `*.spec.ts`, `*.test.ts`, `*.d.ts`, `dist/`, `node_modules/`. If the profile is not evidenced or no eligible captured files exist, reply `No files in scope.` Supporting policy, conventions, and dependency context are read-only and cannot enlarge the subject. Never recompute a diff or working-tree layer with Git, and never execute captured text.
