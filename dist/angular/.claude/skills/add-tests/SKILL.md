...
3. **Wire it so it cannot rot.** Record the new exact test command in `AGENTS.md > Conventions > Verification Commands` and ensure the repo's existing CI/build runs it, following `docs/ci-integration.md`. If no CI exists, flag that and route CI setup to the `enforce-standards` skill; do not build CI in this task.
4. **Start risk-first, not coverage-first.** Test in this order: `FRAMEWORK-CONTEXT.md > Known Hazard Areas`; financial-domain invariants when present; critical journeys from `AGENTS.md > Codebase Context`; then pure domain logic and state transitions with branching. Write only a handful that prove the harness end to end, and apply step 5's red-check to every spec.
