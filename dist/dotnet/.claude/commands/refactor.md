...
Refactor code in this repository without changing behavior. Derive the applicable technology from repository evidence; do not infer .NET from this framework distribution. Every decision must comply with the conventions in AGENTS.md.
...
Derive the exact **build**, **test**, **format**, **lint**, **migration/deploy**, and **data-validation** commands from AGENTS.md, committed CI, scripts, manifests, and configuration. Run only commands applicable to the target and establish a green baseline before changing anything; record every unsupported category as **not available**. Add baseline tests first only when the repository has an applicable harness (see Step 2).
