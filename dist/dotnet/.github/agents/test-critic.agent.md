...
- Use repository evidence to establish whether the .NET test profile applies, then scope to its changed test files (`git diff --name-only HEAD`, `*.cs` test files — `*Tests.cs`/`*Test.cs` or under a `*.Tests` project) unless the user names specific files. If the profile is not evidenced, reply `No test files in scope.` Use `git diff HEAD -- <file>` so you see what was added vs what existed.
