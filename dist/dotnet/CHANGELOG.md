...
- Quick Start now uses the supported installer from the matching .NET distribution, routes stamped
  targets to the upgrade checklist, and preserves the commit plus developer bootstrap/adopt handoff.
...
- The .NET endpoint, entity, service-registration, and warehouse-load skills now derive their
  ...
  operations; the framework ships no placeholder that could overwrite it. Skill refresh decodes
  BOM-less UTF-8 explicitly so non-ASCII project examples survive Windows PowerShell 5.1 updates.
...
NetArchTest is a scaffoldable backstop and becomes enforcement only after you wire it into blocking
CI. Copilot CLI evidence is dated, while VS Code hooks are Preview, off by default,
...
lifecycle, select `dotnet` explicitly; ordinary .NET and mixed-repository detection are unchanged.
