...
- **Feature** — *add / implement / create / build new …*: design check first (affected layers, files to create/modify, failure modes, test strategy — pick levels per `Conventions > Testing` / the Test shape heuristic, say which this change needs and why, and flag missing infrastructure via `add-tests` suite-bootstrap mode) → decompose into ordered subtasks, running `dotnet build` + `dotnet test` after each → Boy Scout every touched file → self-review against Conventions → present what was built and tested. Honour Leanness: no new interface/abstraction without a second consumer in this change-set.
