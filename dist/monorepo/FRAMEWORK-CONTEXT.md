...
>
> **Maintenance**: Every section is drafted by `/bootstrap` from this repo's code. Drafted sections open with an auto-draft comment and cover only what this repo's code shows — the cross-repo half (why a convention exists org-wide, what other services consume, what the backend does, a library's full surface) still needs a maintainer. Edit any section freely; `/bootstrap` never overwrites maintainer-written content. "Detected Framework Packages" and "Known Hazard Areas" are also refreshed by `/docs-sync`.
>
...
<!-- List the shared NuGet and npm packages the team maintains.
...
Template entries — a .NET (NuGet) shape and an Angular (npm) shape:
...
<!-- If applicable: how is tenancy modeled, and where are boundaries enforced?
     - Backend: tenant boundaries, data isolation, conventions for new tenant-aware features
     - Frontend: tenant resolution (subdomain / path / header), tenant-scoped routing, theming,
       feature flags, how tenant context is provided to components / services -->
...
<!-- Patterns for service-to-service and backend/frontend communication within the org:
     - HTTP conventions (base URL, versioning, auth headers, retry, idempotency)
...
_Not yet populated. `/bootstrap` drafts this from observed HttpClient/interceptor/message-bus/correlation wiring; a maintainer adds the org-wide conventions._
...
<!-- Auto-populated by /bootstrap and /docs-sync.
     Lists the framework packages this repo references, with version — across both stacks.
     Helps the AI give version-aware advice and flag drift. -->

<!-- DETECTED_FRAMEWORK_PACKAGES_PENDING: run /bootstrap to populate. -->

| Package | Version | Source (*.csproj / Directory.Packages.props / package.json) |
|---------|---------|-------------------------------------------------------------|
