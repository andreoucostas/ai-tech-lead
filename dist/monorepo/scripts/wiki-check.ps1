...
if($vals.name-ne$stem){Fail "$($f.Name): name must equal filename stem"};if($vals.type-notin@('gotcha','context','recipe','failed-approach')){Fail "$($f.Name): invalid type"};if($vals.status-notin@('verified','suspected','unverified')){Fail "$($f.Name): invalid status"};if($vals.'last-verified'-eq'never'){if($vals.status-eq'verified'){Fail "$($f.Name): verified entry cannot use last-verified: never"}}elseif($vals.'last-verified'-notmatch'^\d{4}-\d{2}-\d{2}$'){Fail "$($f.Name): invalid last-verified"}else{try{$d=[datetime]::ParseExact($vals.'last-verified','yyyy-MM-dd',[Globalization.CultureInfo]::InvariantCulture);if($d-lt[datetime]::Today.AddDays(-90)){Warn "$($f.Name) last verified $($vals.'last-verified')"}}catch{Fail "$($f.Name): invalid last-verified"}}
