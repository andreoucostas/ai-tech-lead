...
> Boy Scout scope and repository-evidenced verification commands live in the calling command
> (because they vary: `/fix` scopes Boy Scout to blast radius, etc.).
...
- **Close with a Verification & confidence line** (calibration): in one or two lines, separate what
  you *actually verified by running it* (name the applicable build, test, format, lint, and
  data-validation commands, plus only safely authorized migration/deploy validation) from
  categories that are **not available**, manual/CI-only, and
  anything otherwise unverified. "Looks correct" is not verification; a check you ran and watched
  pass is. This is deliberate — it counters the well-documented tendency to feel more done than
  the work is.
