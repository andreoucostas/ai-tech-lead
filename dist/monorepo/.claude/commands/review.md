...
- `test-critic` — audits the test/spec changes for integrity: would each test/spec actually fail if the code broke? Catches over-mocking, tautological/weak assertions/expectations, missing paths, and nondeterminism.
...
Run the touched stack's gates — .NET: `dotnet build` and `dotnet test` (plus `dotnet format --verify-no-changes` to catch formatter drift); Angular: `ng build` and `ng test --watch=false --browsers=ChromeHeadless` (plus `ng lint` if configured). Do not trust that the code being reviewed already passes. Record any failures as high-severity issues.
...
- **Security**: injection, XSS, data exposure, auth bypass, sensitive data in logs — auditors do not check these.
- **Test quality**: build on `test-critic`'s findings — confirm the new tests/specs would fail if the code broke, and that error/edge paths are covered. Treat any "would pass against broken code" test/spec as a high-severity issue.
