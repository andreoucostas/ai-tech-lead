...
Refactor code in this mixed .NET + Angular codebase without changing behavior. Every decision must comply with the conventions in CLAUDE.md.
...
Run the touched stack's gates — .NET: `dotnet build` and `dotnet test`; Angular: `ng build` and `ng test --watch=false --browsers=ChromeHeadless`. They must pass before changing anything. If tests don't exist for the code being refactored, write baseline tests FIRST (see Step 2).
...
- After each meaningful change, run the touched stack's gates — .NET: `dotnet build` and `dotnet test`; Angular: `ng build` and `ng test --watch=false --browsers=ChromeHeadless`
...
Run the touched stack's gates — .NET: `dotnet build`, `dotnet test`, and `dotnet format`; Angular: `ng build`, `ng test --watch=false --browsers=ChromeHeadless`, and `ng lint` (if configured). All must pass. No behavior should have changed.
