...
3. **Locate the repo root** — find the `.sln` file (.NET side) and `angular.json` (or `nx.json`/`project.json` for Nx — Angular side); in this mixed repo both live under one root. All paths are relative to that root.
