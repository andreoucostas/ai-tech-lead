...
- Which layers are affected — .NET: domain, application, API, infrastructure; Angular: models, services, state, components, routing?
...
### Data Flow
[how data moves through the system for this feature — .NET: request → controller → application → domain → infrastructure; Angular: API → service → state → component → template]
