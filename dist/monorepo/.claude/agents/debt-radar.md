...
2. Take the caller's input. It is one of: a file path, a list of paths, or a feature-area name (e.g. `Auth`, `Reporting`, `Billing` for .NET; `auth`, `reporting` for Angular).
...
|----|-------|----------|--------|-------|------------------|
| DEBT-NNN | ... | High | S | path/to/Foo.cs:42 or path/to/foo.ts:42 | Yes — same file as your work |
