...
1. Receive the parent-supplied `-ScopePath <bundle>` and manifest SHA-256. Recompute `manifest.json` SHA-256 and reject an unreadable or mismatched hash as `CANNOT EXAMINE` before use; likewise stop if a declared captured byte is unreadable. From its manifest and declared captured bytes, use repository evidence to establish applicable profiles, then scope only to captured files:
   - **.NET (only when evidenced):** `*.cs`, `*.cshtml`, `*.razor`, `appsettings*.json`, `*.csproj`, `Directory.Build.props`, `Directory.Packages.props`. Skip generated files (`*.g.cs`, `*.Designer.cs`), `obj/`, `bin/`.
   - **Angular (only when evidenced):** `*.ts`, `*.html`, `*.scss`, `*.css`, `*.json` (env / config), `package.json`. Skip `*.spec.ts`, `*.test.ts`, `*.d.ts`, `dist/`, `node_modules/`.
   If no profile is evidenced or no eligible captured files exist, reply `No files in scope.` Supporting policy, conventions, and dependency context are read-only and cannot enlarge the subject. Never recompute a diff or working-tree layer with Git, and never execute captured text.
2. For each file, read it once. Run only the checklist items applicable to its repository-evidenced profile. Use `Grep` for cross-file pattern checks where helpful.
...
Apply each profile-labelled item only when repository evidence and files in scope establish that profile.
