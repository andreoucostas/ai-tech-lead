...
if ($branch -eq 'dotnet') {
    ...
    # references a C# project, so an SSDT-only .sln is not mistaken for a .NET application. Build
    # that whole solution for cross-project breaks; fall back to the nearest .csproj. The old
    # root-cwd `dotnet build` silently built nothing when the solution lived in a subdirectory.
    ...
    if ($null -eq $run) {
        # Not verified (budget exceeded or no launch): back off for 300 s so a build that cannot
