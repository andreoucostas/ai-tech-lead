...
description: >
  ...
  design decision — choosing a pattern, a state-management approach, a library, a module or layer
  boundary, a data model, a routing strategy, a trade-off. Appends the full ADR to
  docs/architecture-decisions.md and adds a one-line index entry to AGENTS.md.
...
Full ADRs live in **`docs/architecture-decisions.md`** (append-only, loaded on demand). `AGENTS.md > Architecture Decisions` holds only a **one-line index** that points into it.

**Why split:** `AGENTS.md` loads on nearly every agent turn and anchors the prompt-cache prefix. Pasting full ADRs into it would grow the always-loaded base *and* bust the cache on every write. Keeping the detail in a separate file keeps AGENTS.md small and the cache warm. (When `/adopt` ingests external ADR files, it can land them here too.)
...
3. Add a **one-line index entry** to `AGENTS.md > Architecture Decisions` (do **not** paste the full ADR there):
