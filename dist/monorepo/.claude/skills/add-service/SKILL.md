...
description: >
  ...
  DO NOT USE FOR: adding a method to an existing service (ordinary work — follow the AGENTS.md
...
Match the conventions in AGENTS.md > Conventions > API/HTTP and > State Management. If the service is a signal-based store, see also the `add-signal-store` skill for state-shape rules.
