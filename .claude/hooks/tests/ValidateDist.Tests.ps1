...
function Start-ValidateDistChild {
    ...
    # generated by this driver and contain no embedded quotes; quoting each one preserves case
    # names and self paths with spaces on both native PowerShell hosts.
...
if (-not $Only) {
    ...
    if (-not $DriverProbe) {
        # Exercise the real parent/child boundary before the normal case queue. The synthetic child
        # emits one coherent success summary twice: once with exit 0 (green control), then with exit
    ...
    if ($DriverProbe) {
        $caseNames = @('__driver-boundary-probe__')
    } else {
    ...
    while ($queue.Count -gt 0 -or $running.Count -gt 0) {
        ...
        foreach ($r in @($running | Where-Object { $_.Child.Proc.HasExited })) {
...
try {
    ...
    It 'case 17b: a missing AGENTS.md import fails the full validator' { Assert-Case 'missing-agents-import' { param($d) $p=Join-Path $d 'CLAUDE.md'; $t=[IO.File]::ReadAllText($p); [IO.File]::WriteAllText($p,[regex]::Replace($t,'(?m)^@AGENTS\.md\r?\n','')) } 'CLAUDE.md is missing required import @AGENTS.md.' 'carrier-import' }
    ...
    It 'case 19: prose after valid finite-registry citations does not become a heading' { Assert-Case 'citation-prose' { param($d) $p=Join-Path $d 'README.md'; [IO.File]::AppendAllText($p,"`nAGENTS.md > Conventions wins on any conflict.`nAGENTS.md > Boy Scout Rule before considering the work complete.`n[AGENTS.md](./AGENTS.md) > Conventions.`n") } 'all registered section-path references resolve' 'section-path' -Green }
