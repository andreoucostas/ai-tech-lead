...
# policy is therefore asserted on both installer twins, including every refusal path.
...
function Invoke-LicenseInstaller {
    param([string]$Twin, [string]$Target, [string]$Dist = 'dotnet')
    $inst = Join-Path $repoRoot "dist/$Dist/scripts/install.$Twin"
    $ef = [IO.Path]::GetTempFileName()
    try {
        if ($Twin -eq 'ps1') { $out = & (Get-PsExe) -NoProfile -File $inst $Target 2>$ef }
        else { $out = & $bash $inst $Target 2>$ef }
...
foreach ($twin in @('ps1', 'sh')) {
    if ($twin -eq 'sh' -and -not $bash) { Skip "licence delivery ($twin)" 'no bash on this host'; continue }

    foreach ($dist in @('dotnet', 'angular', 'monorepo')) {
        It "greenfield creates the licence and notice with the shipped content ($dist/$twin)" {
            $t = New-LicenseTarget
            try {
                $r = Invoke-LicenseInstaller -Twin $twin -Target $t -Dist $dist
        ...
        It "brownfield without a legal collision creates both files ($dist/$twin)" {
            $t = New-LicenseTarget
    ...
    foreach ($rel in @($licenseRel, $noticeRel)) {
        It "brownfield refuses a conflicting $rel without changing it ($twin)" {
            $t = New-LicenseTarget
            Set-Content -LiteralPath (Join-Path $t 'TECH_DEBT.md') -Value '# existing debt' -Encoding UTF8
            $p = Join-Path $t $rel
            ...
            try {
                $r = Invoke-LicenseInstaller -Twin $twin -Target $t
                ...
                Write-Host "[observed refusal] twin=$twin exit=$($r.Exit) path=$rel message=$($r.Out.Trim())"
            } finally { Remove-Item -Recurse -Force $t -ErrorAction SilentlyContinue }
        }
    }

    It "update replaces a stale framework-owned notice ($twin)" {
    ...
    It "update leaves an LF-normalised-identical licence byte-untouched ($twin)" {
        $t = New-LicenseTarget -Update
    ...
    It "update refuses a notice whose ownership marker was removed ($twin)" {
        $t = New-LicenseTarget -Update
        $p = Join-Path $t $noticeRel
        ...
        try {
            ...
            Write-Host "[observed refusal] twin=$twin exit=$($r.Exit) path=$noticeRel message=$($r.Out.Trim())"
        } finally { Remove-Item -Recurse -Force $t -ErrorAction SilentlyContinue }
    }
}
