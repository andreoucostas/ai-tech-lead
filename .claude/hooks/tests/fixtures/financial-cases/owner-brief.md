...
- The two input terms are unrounded allocation ratios. Their expected aggregate is 0.008 with an
  absolute tolerance of 0.000001. Rounding belongs at the later presentation boundary, which is not
  included in this fixture.
- Re-delivery of the same request identifier must produce at most one effect.
- The report includes only rows marked current. For the supplied rows, its expected total is 100.
