...
# Usage: pwsh install.ps1 [-Stack dotnet|angular] C:\path\to\target-repo
#
...
$usage = 'Usage: pwsh install.ps1 [-Stack dotnet|angular] C:\path\to\target-repo'
...
$reason = ''
if ($Stack) {
    if ($Stack -ne 'dotnet' -and $Stack -ne 'angular') { Die "Unknown stack '$Stack' (expected: dotnet or angular)." }
    $reason = '-Stack flag'
}
else {
    ...
    if (Test-Path -LiteralPath $vf -PathType Leaf) {
        ...
        if (-not $tmpl) { Die "Existing install at '$tgt', but .claude/framework-version.json has no readable ""template"" value — pass -Stack dotnet|angular." }
        if ($tmpl -ne 'dotnet' -and $tmpl -ne 'angular') { Die "Existing install names an unknown stack ""$tmpl"" in .claude/framework-version.json — pass -Stack dotnet|angular." }
        $Stack = $tmpl
        ...
        if ($hasDotnet -and $hasAngular) {
            Die ("Both .NET (*.csproj/*.sln) and Angular (angular.json) markers found in '$tgt'.`n" +
                'Mixed-repo / monorepo installs arrive in Phase 4. For now choose one explicitly: -Stack dotnet|angular.')
        }
        ...
        elseif ($hasAngular) { $Stack = 'angular'; $reason = 'auto-detected (found angular.json)' }
            ...
            Die ("Could not determine the stack for '$tgt': no *.csproj/*.sln and no angular.json in the target root or two levels below.`n" +
                'Pass it explicitly: -Stack dotnet|angular.')
        }
    }
}
