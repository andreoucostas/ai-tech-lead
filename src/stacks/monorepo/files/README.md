...
| `scripts/` | Host-agnostic helpers include `metrics.{sh,ps1}` and retired `impact-run.{sh,ps1}` compatibility tombstones; `ci/` contains NetArchTest/dependency-cruiser scaffolding to wire in consumer CI. |
...
- **GitHub Copilot CLI** — on CLI 1.0.80 (observed 2026-08-20), trusted-folder hooks run locally when the interpreter resolves; guard, build/type feedback, and mutable telemetry remain limited to their registered events.
