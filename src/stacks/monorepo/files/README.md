...
A working template that turns Claude Code and GitHub Copilot into a tech lead for your mixed .NET + Angular codebase. Clone it, run one command, and get AI-driven development with built-in conventions, verification, and continuous improvement.

This install carries **both stacks' rails** in one repo — the .NET backend and the Angular frontend each get their own conventions, hooks, and skills, driven from a single source of truth. The Angular rails target **Angular 17+** (standalone components, signals, new control flow, `inject()`, `takeUntilDestroyed`); the .NET rails target a modern .NET solution. Bootstrap auto-detects your versions and adjusts conventions accordingly.
...
1. **Copy the files in:** `pwsh install.ps1 <target-repo-path>` (or `bash install.sh <target-repo-path>`). The root installer detects a mixed .NET + Angular repo and installs both stacks' rails; it also detects the mode itself: **greenfield** (plain copy), **brownfield** (the target already has AI tooling — the originals its copy would overwrite are moved to `docs/pre-adoption/` and `.claude/adoption-pending.json` is written), or **update** (target already stamped with `.claude/framework-version.json` — consumer-owned content files are left untouched).
...
3. **One command instead of hours hand-writing the AI's context.** `/bootstrap` (or `/adopt`) analyses both stacks — .NET: architecture, domain, DI, API surface, testing, code quality; Angular: modules, state management, components, RxJS, the API layer, testing — then writes `CLAUDE.md`, `TECH_DEBT.md`, `AGENTS.md`, and `copilot-instructions.md`. You stop hand-authoring AI context — it's derived from the real codebase.
...
5. **Compile and type errors caught the moment they're written.** A PostToolUse hook runs an incremental `dotnet build` after every `.cs` write and `tsc --noEmit` (1–2 s) after every `.ts` write, so an error surfaces on the next step instead of compounding across ten files.

6. **Bad writes blocked deterministically — no review round-trip.** A PreToolUse hook hard-blocks any write that adds a suppression (`#pragma warning disable` in C#; `// eslint-disable`, `@ts-ignore`, `@ts-nocheck` in TypeScript) or a hardcoded secret. Enforced by code, not by remembering to check.

7. **Natural language routes to the right workflow — no slash commands to memorise.** Typing *"the export endpoint is broken"* — or *"the export button is broken"* — auto-injects the `/fix` rails (regression-test-first, blast-radius cleanup). The seven workflows are still available as explicit slash commands when you want deterministic routing.

8. **Common tasks can't be done wrong.** Skills encode the correct end-to-end recipe (add-endpoint: domain → service → DTO → validator → thin controller → integration test; add-component: scaffold → routing → models → service wiring → state choice → tests). Juniors get senior-level scaffolding; the agent follows *your* recipe, not a generic one.
...
Copy the following into your **monorepo root** (where your `.sln`/`.csproj` and `angular.json` live):
```
...
docs/defaults.md                    → greenfield .NET + Angular conventions (used until /bootstrap runs)
...
| `.claude/skills/*/SKILL.md` | Auto-discovered Common Tasks recipes (add-endpoint, add-entity, register-service, add-component, add-service, add-lazy-route, add-signal-store, add-tests, perf, dependency-audit, create-adr, enforce-architecture, enforce-standards). Body loads only when triggered. Mirrored to `.github/skills/` for Copilot. |
...
| `.claude/hooks/*.sh` | SessionStart context preload, UserPromptSubmit intent router, **PreToolUse guard** (blocks warning-suppressions & secrets), PostToolUse build/type-check trigger and audit trail, Stop Boy Scout scanner. Each has a `.ps1` twin for Windows-only teams. |
| `.claude/settings.json` | Registers hooks for Claude Code: SessionStart, UserPromptSubmit, PreToolUse (`guard` before `.cs`/`.ts` writes), PostToolUse (`dotnet build` + audit trail after `.cs` writes, `tsc --noEmit` after `.ts` writes), and Stop. |
...
| `scripts/` | Host-agnostic helpers: `install.{sh,ps1}` (install into a target repo), `docs-sync-check.{sh,ps1}` (CI guardrail), `sync-agent-files.{sh,ps1}` (skills mirror), `build-architecture-html.{sh,ps1}`, `metrics.{sh,ps1}` + `impact-run.{sh,ps1}` (impact harness), `ci/` samples (Bitbucket Pipelines, NetArchTest, dependency-cruiser). |
...
2. **Execute in verified subtasks** — run the gates of the stack(s) the change touches after each (.NET: build + test + format; Angular: build + test + lint)
...
| `PreToolUse` (Write/Edit) | Before every `.cs` / `.ts` write | **Hard-blocks** the write if it adds a suppression (`#pragma warning disable` in C#; `// eslint-disable`, `@ts-ignore`, `@ts-nocheck` in TypeScript) or a hardcoded secret (private key, cloud token, credential literal). Deterministic enforcement of Verification Rule #7. Runs in Claude Code **and** Copilot CLI. |
| `PostToolUse` (Write/Edit) | After every `.cs` / `.ts` write | For `.cs`: runs solution-level incremental `dotnet build` — catches compilation errors before they compound. For `.ts`: runs `tsc --noEmit` (1–2 s) — catches type errors before they compound. A second handler appends an SR 11-7 / DORA audit-log line. |
| `Stop` | End of every turn (Claude Code only) | Scans modified files for the always-apply Boy Scout patterns (.NET: async without `CancellationToken`, interpolated logger calls, EF read queries without `AsNoTracking()`, excess null-forgiving `!`; Angular: manual `ngOnDestroy` + `subscribe`, nested `subscribe`, `any`, commented-out code blocks); soft-warns the model. `OnPush` is intentionally excluded — switching a component to `OnPush` is a semantic change, not a drive-by cleanup. Copilot has no equivalent event. |

The router is the key piece. **In Claude Code**, a developer who types *"the export endpoint is broken"* (or *"the export button is broken"*) gets the `/fix` rails (regression-test-first, blast-radius Boy Scout) auto-injected per-prompt, without typing a slash command. **In Copilot**, the same vocabulary is preloaded once per session and the model self-classifies — works well with top-tier models, less reliable with smaller ones. Either way, the seven workflows are also invokable explicitly as slash commands (`/feature`, `/fix`, …) for deterministic routing.
...
| `solid-check` | Audits a diff against CLAUDE.md > SOLID — the five principles (.NET: a literal interface per injected service; Angular: an abstraction/token per injected service). Read-only. | `/review` Step 1; ad-hoc |
...
Full `dotnet test`, `ng build`, and `ng test` run inside command workflows, not as hooks — they're too slow for per-write execution.
...
- **GitHub Copilot CLI** (GA Feb 2026) — runs `.github/hooks/hooks.json` hooks **locally on your machine**: the PreToolUse guard, the `dotnet build` / `tsc --noEmit` checks, and the SR 11-7 / DORA audit trail all fire. (Only the *cloud-agent* half of hooks.json is inert on Bitbucket — the CLI half works.)
...
- **Dependencies**: Dependabot is GitHub-only — use **Renovate** (self-hostable) or the `dependency-audit` skill's CI fallback (.NET: `dotnet list package --vulnerable --include-transitive`; Angular: `npm audit --audit-level=high`).
- **SAST**: CodeQL is GitHub-only — run **Semgrep** or **SonarQube** (C# and JS/TS) in CI and publish via Code Insights.
