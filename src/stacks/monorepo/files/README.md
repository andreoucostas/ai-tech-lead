...
Target your repository's **Git root**. `*.csproj`, Angular workspace/configuration, and warehouse signals are profile markers when present; a `.sln` alone may contain only SSDT/`*.sqlproj` projects and is not .NET application evidence. None is required merely to choose this distribution:
...
From the matching incoming `dist/monorepo` directory in a fresh framework checkout, use PowerShell 7
