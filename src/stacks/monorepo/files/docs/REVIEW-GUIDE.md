...
- **Hook-script input/output fixtures (not host firing):** `echo '{"prompt":"the export endpoint is broken"}' | bash .claude/hooks/route-prompt.sh` → should print the `/fix` rails. The guard, per stack:
  - .NET: `echo '{"tool_name":"Write","tool_input":{"file_path":"Foo.cs","content":"#pragma warning disable CS8602"}}' | bash .claude/hooks/guard.sh; echo $?` → emits the Claude Code block path (exit 2).
  - Angular: `echo '{"tool_name":"Write","tool_input":{"file_path":"a.ts","content":"// @ts-ignore"}}' | bash .claude/hooks/guard.sh; echo $?` → emits the Claude Code block path (exit 2).
  These direct commands prove parser and output-shape behavior only; they do not prove that a client fires the event or consumes the output.
