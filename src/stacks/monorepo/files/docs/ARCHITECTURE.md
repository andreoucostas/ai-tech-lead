...
| Command | Purpose |
|---------|---------|
| `/bootstrap` | One-time: analyse the codebase across both stacks (.NET: 8 parallel passes A1–A8, incl. financial-domain invariants + skill discovery; Angular: 7 parallel passes A1–A7: modules, state, components, RxJS, API, build/test/quality, skill discovery), populate CLAUDE.md + TECH_DEBT.md, generate AGENTS.md + copilot-instructions, sync skills |
...
| Agent | Role | Model |
|-------|------|-------|
| `bootstrap-pass` | One analysis pass (.NET A1–A8 / Angular A1–A7) during `/bootstrap` | inherit (strong) |
...
sequenceDiagram
    ...
    H-->>A: guard — BLOCK if write adds a suppression (#pragma warning disable / eslint-disable / @ts-ignore) or a secret
    A->>H: PostToolUse (after .cs or .ts write)
    H-->>A: .cs → dotnet build (+ SR 11-7/DORA audit line); .ts → tsc --noEmit (output only on failure)
...
- **SOLID (mandatory)** — literal classic SOLID: an abstraction for **every injected service** (DIP — .NET: an interface `IFoo` + `Foo`; Angular: an `abstract class` token or `interface` + `InjectionToken`) plus SRP/OCP/LSP/ISP. Enforced semantically by `solid-check`; deterministic dependency-direction backstop is **NetArchTest** (.NET) / **dependency-cruiser** or `eslint-plugin-boundaries` (Angular) in CI. Data carriers are exempt.
...
- **Evals** — `tests/evals/cases.yaml` is the executable spec of framework behavior (Verification, Leanness, SOLID/DIP, Boy Scout / `takeUntilDestroyed`, no-defensive-overcoding, `bypassSecurityTrust` safety). Deterministic regex + model-graded rubric.
