...
3. **Volatile content lives in hooks, not in `AGENTS.md`.** `SessionStart` injects branch + recent commits + debt heat at session start; `UserPromptSubmit` injects routed-intent rails; `PostToolUse` injects type-check output. All of these append AFTER the AGENTS.md prefix, preserving cache integrity.
