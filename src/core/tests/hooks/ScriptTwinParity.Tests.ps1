...
function TemplateFixture { $r=Temp template;CopyPair template-checks $r;Put (Join-Path $r CLAUDE.md) "version: 1.2.3`r`n## Verification Rules`r`nSame`r`n## Leanness`r`nSame`r`n## SOLID`r`nSame`r`n## Boy Scout Rule`r`nSame`r`n## Agentic Workflow`r`n### 1. Classify the intent`r`nSame`r`n### 2. Continue`r`n## Common Tasks`r`n- ``alpha`` — first → second`r`n- ``beta`` - plain separator`r`n";Put (Join-Path $r AGENTS.md) "## Verification Rules`r`nSame`r`n## Leanness`r`nSame`r`n## SOLID`r`nSame`r`n## Boy Scout Rule`r`nSame`r`n## Agentic Workflow`r`n### 1. Classify the intent`r`nSame`r`n### 2. Continue`r`n## Common Tasks`r`n- ``alpha`` — first → second`r`n- ``beta`` - plain separator`r`n";Put (Join-Path $r '.claude/framework-version.json') '{"version":"1.2.3"}';Put (Join-Path $r '.github/copilot-instructions.md') "fixture`n";PutBom (Join-Path $r '.claude/hooks/probe.ps1') "# probe`n";Put (Join-Path $r '.claude/hooks/probe.sh') "# probe`n";Put (Join-Path $r '.claude/skills/demo/SKILL.md') "# demo`n";Put (Join-Path $r '.github/skills/demo/SKILL.md') "# demo`n";$r }
...
It 'template-checks Common Tasks twins agree on planted inventory failures and edge fixtures' {
    foreach ($case in 'one-sided','duplicate','zero-extraction','absent-one','case-variant','single-slug','absent-both') {
        ...
        try {
            ...
            Assert ($p.Exit-eq$s.Exit) "$case exit mismatch $($p.Exit)/$($s.Exit)"
