...
| **Plan-gate** (plan + clarify before code) | Guaranteed-ish: injected per-prompt by `route-prompt` + Instructed (`§2`) | Injected per-prompt through the single composed entry (CLI ≥ v1.0.65; observed CLI 1.0.80, 2026-08-18) + Instructed | Injected per-prompt if Preview hooks enabled; otherwise instructed only |
| **Security pass** (auth/money/secrets → `/security-review`) | Injected per-prompt by `route-prompt` + Instructed (`§1`) | Injected per-prompt through the single composed entry (CLI ≥ v1.0.65; observed CLI 1.0.80, 2026-08-18) + Instructed | Injected per-prompt if Preview hooks enabled; otherwise instructed only |
