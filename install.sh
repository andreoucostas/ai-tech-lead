#!/usr/bin/env bash
# AI Tech Lead Framework — root installer wrapper (monorepo).
# Usage: bash install.sh [--stack dotnet|angular] /path/to/target-repo
#
...
#   4. both detected      error: mixed-repo/monorepo mode arrives in Phase 4 — pass --stack.
#   5. nothing detected   error: pass --stack.
# Every error exits 2 with an actionable message.
set -euo pipefail

usage="Usage: bash install.sh [--stack dotnet|angular] /path/to/target-repo"
...
valid_stack() { [ "$1" = "dotnet" ] || [ "$1" = "angular" ]; }

reason=""
if [ -n "$stack" ]; then
  valid_stack "$stack" || { echo "Unknown stack '$stack' (expected: dotnet or angular)." >&2; exit 2; }
  reason="--stack flag"
else
  ...
  if [ -f "$vf" ]; then
    ...
    if [ -z "$tmpl" ]; then
      echo "Existing install at '$tgt', but .claude/framework-version.json has no readable \"template\" value — pass --stack dotnet|angular." >&2; exit 2
    fi
    valid_stack "$tmpl" || { echo "Existing install names an unknown stack \"$tmpl\" in .claude/framework-version.json — pass --stack dotnet|angular." >&2; exit 2; }
    stack="$tmpl"
    ...
    if [ "$has_dotnet" -eq 1 ] && [ "$has_angular" -eq 1 ]; then
      echo "Both .NET (*.csproj/*.sln) and Angular (angular.json) markers found in '$tgt'." >&2
      echo "Mixed-repo / monorepo installs arrive in Phase 4. For now choose one explicitly: --stack dotnet|angular." >&2
      exit 2
      ...
      echo "Pass it explicitly: --stack dotnet|angular." >&2
      exit 2
    fi
  fi
fi
